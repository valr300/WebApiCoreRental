...
ADD COLUMN `PricePerWeekReal` DECIMAL(15,2) NULL AFTER `Access`,
CHANGE COLUMN `PricePerWeek` `PricePerWeekSystem` INT NULL DEFAULT NULL ;

ALTER TABLE `Rental`.`Rentals` 
CHANGE COLUMN `PricePerWeekSystem` `PricePerWeekSystem` DECIMAL(15,2) NULL DEFAULT NULL AFTER `Access`;

ALTER TABLE `Rental`.`Rentals` 
ADD COLUMN `ItemNumber` VARCHAR(8) NULL AFTER `RentName`;


ALTER TABLE `Rental`.`RentalStats` 
ADD COLUMN `Invoice` VARCHAR(12) NULL AFTER `InvoiceRentId`,
ADD COLUMN `PricePerWeekReal` DECIMAL(15,2) NULL AFTER `AmountSystem`,
ADD COLUMN `RentalStatscol` VARCHAR(45) NULL AFTER `PricePerWeekReal`,
ADD COLUMN `AmountReal` DECIMAL(15,2) NULL AFTER `RentalStatscol`,
CHANGE COLUMN `PricePerWeek` `PricePerWeekSystem` INT NULL DEFAULT NULL ,
CHANGE COLUMN `Amount` `AmountSystem` INT NULL DEFAULT NULL ;

ALTER TABLE `Rental`.`RentalStats` 
CHANGE COLUMN `AmountReal` `AmountReal` DECIMAL(15,2) NULL DEFAULT NULL AFTER `PricePerWeekReal`,
CHANGE COLUMN `RentalStatscol` `PaymentMethod` VARCHAR(10) NULL DEFAULT NULL ;

ALTER TABLE `Rental`.`RentalStats` 
CHANGE COLUMN `PricePerWeekSystem` `PricePerWeekSystem` DECIMAL(15,2) NULL DEFAULT NULL AFTER `NbDays`,
CHANGE COLUMN `AmountSystem` `AmountSystem` DECIMAL(15,2) NULL DEFAULT NULL AFTER `AmountReal`;
